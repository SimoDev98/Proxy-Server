...
"""set: Socket connections (client-proxy and proxy-remote) store"""

def alive_connections_count():
    """
    Count the number of alive connections.
    Return:
    int: Number of alive connections
    """
    count = 0
...
def start_new_connection(client_connection):
    ...
    global socket_connections
    socket_connections.add(remote_server_connection)
...
def listen_new_connections():
    ...
    """
    global server_socket, connections, keep_listening, socket_connections
    while keep_listening:
        try:
            ...
            socket_connections.add(client_connection)
...
def discard_disconnected_sockets(interval):
    """
    Remove the closed socket connections after the given interval in loop.
    Parameters:
    int: Interval in seconds
    """
    global socket_connections, keep_listening
    while keep_listening:
        sleep(interval)
        for sock in socket_connections:
            if sock.fileno() == -1:
